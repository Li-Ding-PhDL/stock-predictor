...
一个**单文件**（`stock_predictor.py`，约 9000+ 行）的沪深 A 股预测研究平台，用一套统一的
...
建议 **14:30 后**操作，结果含**五维雷达图 + 涨幅/换手率散点图**可视化。
...
### 📊 大宗交易分析（新增）
历史大宗交易明细（折溢率/成交额/买卖方营业部），折溢率着色（溢价>1%=机构主动买/红；折价<-1%=绿）；
...
```
stock-predictor/
├── stock_predictor.py   # 全部功能（单文件，约9000+行）
├── README.md            # 本文件
├── PROJECT.md           # 5分钟工程总览
