...
- **每日信号推送手机**：`dip_daily_report`/`--dip-daily` 扫今日买点+逐只附风险(复用 `_anomaly_risk_one`:ST/亏损/异动/闪崩)→生成**手机友好响应式 HTML**(`每日信号_日期.html`)，并可选推送 `--push`(`pushplus:TOKEN`/`serverchan:SENDKEY`/`https://群机器人webhook`；`_push_message` 只 POST 用户显式给的地址、绝不内置 token)。定时用 `scripts/每日信号推送.bat` + Windows 任务计划程序(每天15:35)。手机看 HTML=发到微信文件传输助手/云盘即可。
