...
- **『暴跌抄反弹』规则策略(第八部分补充10)**：`dip_bounce_backtest`/`dip_bounce_scan`/`dip_daily_report` + `--dip-backtest`/`--dip-scan`/`--dip-daily` + GUI「暴跌抄反弹」页。规则=近N日≥X天跌(+可选回撤≥drop%)(+monthly:价在月均线 below超卖/above趋势)且**末日涨(首阳)→买、持Y日卖**。`_dip_signal_at` 因果(只用≤i)。**默认参数 N=10/X=8/Y=5**(经训练≤2023/测试2024+样本外验证最稳,两段~+2.8%/笔;训练↔测试净收益相关+0.49,说明超卖反弹是真效应、调参有效但仍防过拟合)。条件松(X/N≈0.5)≈抛硬币。**样本偏差(幸存者/选股/扎堆)高估真实收益，约4成会亏**；研究用途、非投资建议。
- **每日信号推送手机**：`dip_daily_report`/`--dip-daily` 扫今日买点+逐只附**公司/交易风险**(`_stock_risk_brief`：异动/避雷 ST/亏损/涨跌停/闪崩 + 财务红旗 亏损/高估值/高PB + `--dip-news` 尽力抓新闻风险词 立案/退市/减持/商誉…联网失败自动跳过)→生成**手机友好响应式 HTML**(`每日信号_日期.html`)，并可选推送 `--push`(`pushplus:TOKEN`/`serverchan:SENDKEY`/`https://群机器人webhook`；`_push_message` 只 POST 用户显式给的地址、绝不内置 token)。`--dip-multi`=多套策略(稳健10/8/5 + 深跌15/10/5+回撤15%+月线下方)一条消息分区推(省推送额度；`presets` 参数)。
- **云端定时推送(不用本机)**：`.github/workflows/daily_signal.yml` 每交易日 15:35(北京) 云端 update-data(baostock)→dip-daily --dip-multi --dip-news →推微信；推送方式走仓库 Secret `PUSH_SPEC`(完整spec,兼容旧`PUSHPLUS_TOKEN`)。PushPlus 2024-08 起需实名收费→改用 Server酱(`serverchan:SENDKEY`,免费5条/天)或企业微信群机器人 webhook。境外连A股源可能不稳,失败推"命中0/数据未更新"。本机稳妥方案见 `scripts/每日信号推送.bat` + 任务计划程序。
