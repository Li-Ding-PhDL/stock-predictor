...
## 2b. 反数据泄漏 Checklist（改任何涉及"喂给模型"的代码前，逐条自查）

> 这是量化里"一眼看出代码有没有偷看未来"的核对清单。每条都对应上面某条红线，改动前后各过一遍。

- [ ] **时序不 shuffle**：训练/验证/测试严格按时间先后切；CV 用前扩窗口 walk-forward，不拿未来折训练。
- [ ] **标准化只在训练集 fit**：`scaler.fit(X_train)` 后 `transform` 验证/测试；绝不在全量/测试集上 fit。
- [ ] **目标不进特征**：预测目标(未来收盘/涨跌)绝不出现在 X 里；窗口只到"基准日及以前"。
- [ ] **外部数据向后对齐**：`merge_asof(direction="backward")`，只取"当天及以前已知"的值。
- [ ] **财报按公告日对齐**：财报/估值若作时序特征，必须用"实际公告日"而非"报告期截止日"(有披露滞后)——当前仅横截面用，未作时序特征。
- [ ] **宏观/新闻/政策低频数据防前视**：GDP/CPI 等发布滞后 2-4 周，只能用"发布那刻已公开"的值；近期新闻无可靠历史，只作当前快照、不入历史训练。
- [ ] **保留朴素基准**：Naive(前值) 与 总是涨(方向基准) 不删，任何"提升"都与之公平对比。
- [ ] **只报真实数字**：`预测对比图` R² 高≠准(明天≈今天)，必看 DA；不得只展示"看起来很准"的一面。
- [ ] **幸存者偏差标注**：回测结论注明只覆盖仍上市标的、会高估。
- [ ] **样本外验证**：新因子/新特征加入后，用 消融实验 / Walk-Forward / 因子IC 看是否带来**统计显著**提升，不显著=噪音，不硬加。
